...
//
// Self-contained capture backend that integrates into RustDesk's scrap crate.
// It links the `libdrmtap-sys` crate, which statically embeds the libdrmtap C
// sources (no shared library to install), and spawns a privileged helper
// (drmtap-helper, CAP_SYS_ADMIN + seccomp) for capture without root.
//
...
//
// Tested on:
//   - Intel Meteor Lake (Arc, i915) dual 3840×2160 via EGL detiling
//     of the compressed INTEL_4_TILED_MTL_RC_CCS_CC modifier
