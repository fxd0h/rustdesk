...
pub fn get_cursor() -> ResultType<Option<u64>> {
    // On Wayland, the DRM backend captures the compositor's real hardware cursor
    // (which changes shape correctly). XFixes only sees the X cursor and is stale
    // over native Wayland apps, so prefer the DRM cursor when available. Its id is
    // a content hash, so it changes whenever the cursor shape changes.
    if !is_x11() {
...
pub fn get_cursor_data(hcursor: u64) -> ResultType<CursorData> {
    // Wayland: serve the real hardware cursor captured by the DRM backend
    // (correct shape, updates dynamically), instead of the stale XFixes cursor.
    if !is_x11() {
        if let Some(c) = scrap::drm_cursor() {
