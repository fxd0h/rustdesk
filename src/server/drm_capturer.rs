...
/// Mixes the hotspot into the wire id on top of the reader's fold, so the client's shape cache
/// sees the corrected cursor as a new id. A collision, with this shape's own wire id or with
/// another shape's, is as unlikely as between any two 64-bit ids, and nothing depends on it
/// never happening: it would serve one cached bitmap for another until the next shape change.
