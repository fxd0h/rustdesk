...
/// The layout generation whose uinput range the device runs, and a count of the ranges it
/// acknowledged. The DRM cursor calibration measures only under its own generation and against a
/// sample injected under the current count: a stale range maps the point elsewhere, and two stops
/// share that error, so agreement cannot reject it.
#[cfg(all(target_os = "linux", feature = "drm"))]
